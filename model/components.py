...
def setup_dnn_layers(input_dim, hidden_dims, output_dim, 
                     dropout_rate = 0, batch_norm = False):
    all_dims = [input_dim] + hidden_dims + [output_dim]
    layers = []
    for k in range(len(all_dims)-2):
        layers.append((f'h{k}', nn.Linear(all_dims[k], all_dims[k+1])))
        init_weights(layers[-1][1])
        layers.append((f'a{k}', nn.ReLU()))
        if dropout_rate > 0:
            layers.append((f'dropout{k}', nn.Dropout(dropout_rate)))
        if batch_norm:
            layers.append((f'bn{k}', nn.BatchNorm1d(all_dims[k+1])))
    layers.append((f'h{len(all_dims)-2}', nn.Linear(all_dims[-2], all_dims[-1])))
    init_weights(layers[-1][1])
    dnn_modules = nn.Sequential(OrderedDict(layers))
    return dnn_modules
