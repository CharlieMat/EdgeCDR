...
#         return super().wrap_batch(batch)
    ...
    def get_loss(self, feed_dict: dict, out_dict: dict):
        ...
        preds, reg = out_dict["pos_preds"].view(-1), out_dict["pos_reg"] # size (B,)
    ...
    def forward(self, feed_dict: dict, return_prob = False):
        '''
    ...
    def forward_all(self, feed_dict):
        '''
        Called during evaluation or prediction
        '''
        # forward for positive samples
        pos_out = self.get_forward({k[5:]: v for k,v in feed_dict.items() if "user_" in k or "item_" in k})
        out_dict = {"pos_"+k: v for k,v in pos_out.items()}
        # forward for negative samples
        if self.loss_type != "regression":
            neg_out = self.get_forward({k[5:]: v for k,v in feed_dict.items() if "user_" in k or "negi_" in k})
            for k,v in neg_out.items():
                out_dict["neg_" + k] = v
        return out_dict
