...
class BaseReader(Dataset):
    ...
    def _load_vocab(vocab_file, meta):
        ...
        '''
        self.do_print("Load vocab from : " + vocab_file)
    ...
    def _read_data(self, args):
        ...
        for phase in ["train", "val", "test"]:
            self.do_print(f"Loading {phase} data file", end = '\r')
            self.data[phase] = pd.read_table(args.data_file + phase + ".tsv", 
                                         sep = '\t', engine = 'python')
            self.do_print(f"Loading {phase} data file. Done.")
        self.do_print(self.data["train"].head())
#         self.data = dict()
