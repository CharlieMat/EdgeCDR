...
def holdout_data_sequential(df, holdout_type = "warm", 
                            ratio = [0.8,0.1,0.1], 
                            min_hist_len = 5):
    ...
    if holdout_type == "leave_one_out":
        ...
        '''
        for u,H in tqdm(user_hist.items()):
            if len(H) > min_hist_len:
                nTest = max(int(len(H) * ratio[2]), 1)
                nVal = max(int(len(H) * ratio[1]), 1)
                test_samples = np.random.choice(H,nTest)
                test_indices.iloc[test_samples] = True
                val_indices.iloc[np.random.choice(list(\
                                set(H)-set(test_samples)),nVal)] = True
    elif holdout_type == "cold":
        '''
